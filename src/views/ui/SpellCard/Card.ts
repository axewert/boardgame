...
export class Card extends BasicViewComponent<UI.SpellCardProps>{
  protected domElement: HTMLElement
  protected name = 'card'
  html({spell}: UI.SpellCardProps) {
    const title = spell.name.split('_').join(' ')
    return `
      <div class="card">
        <div class="card__bg"></div>
        <div class="card__image"></div>
        <div class="card__text">${spell.description}</div>
